...
import { Sword, AlertCircle, Plus, Users, Share2, Globe, Link, User, Users2, Flame, Tent, Battery, Key, ShieldCheck, ExternalLink } from 'lucide-react';
import Gun from 'gun';
...
const App: React.FC = () => {
  ...
  const handleConnectAccount = async () => {
    try {
      if ((window as any).aistudio) {
        ...
        setHasKey(true); // Proceeding as if successful per guidelines
      }
    } catch (e) {
  ...
  const handleStartAdventure = async () => {
    ...
    try {
      ...
      if (err.message === "API_KEY_EXPIRED") {
        setError("Your API key essence has faded. Please reconnect your Google account.");
        setHasKey(false);
      } else {
  ...
  const handleSendMessage = async (text: string, charId: string) => {
    ...
    try {
      ...
      if (err.message === "API_KEY_EXPIRED") {
        setError("API Session expired. Reconnect your account.");
        setHasKey(false);
      } else {
    ...
    <div className="min-h-screen flex flex-col h-screen overflow-hidden bg-slate-950">
      <header className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex items-center justify-between z-10 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          {hasKey && (
            <div className="ml-2 flex items-center gap-1 bg-sky-500/10 px-2 py-0.5 rounded border border-sky-500/30">
              <ShieldCheck size={10} className="text-sky-400" />
              <span className="text-[9px] font-bold text-sky-400 tracking-tighter uppercase">Sanctum Linked</span>
            </div>
          )}
        </div>
      ...
      <main className="flex-1 flex overflow-hidden">
        {!gameState.sessionMode && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-slate-950 p-8 text-center overflow-y-auto">
            <div className="max-w-4xl w-full space-y-12 py-12 animate-in fade-in zoom-in duration-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <button 
                  ...
                  className="group relative bg-slate-900 border border-slate-800 hover:border-amber-500/50 rounded-3xl p-8 flex flex-col items-center justify-center gap-6 transition-all hover:scale-[1.02] shadow-2xl overflow-hidden min-h-[300px]"
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-500 max-w-[200px] mx-auto">A solitary adventure recorded only in your local chronicle.</p>
                  </div>
                </button>

                <button 
                  onClick={() => handleSelectMode('online')}
                  className="group relative bg-slate-900 border border-slate-800 hover:border-sky-500/50 rounded-3xl p-8 flex flex-col items-center justify-center gap-6 transition-all hover:scale-[1.02] shadow-2xl overflow-hidden min-h-[300px]"
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-500 max-w-[200px] mx-auto">Join or host a real-time saga with other players across the realm.</p>
                  </div>
                </button>
              </div>

              <div className="pt-8 border-t border-slate-900 flex flex-col items-center gap-4">
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 max-w-md w-full">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${hasKey ? 'bg-emerald-500/10 text-emerald-500' : 'bg-sky-500/10 text-sky-500'}`}>
                        <Key size={20} />
                      </div>
                      <div className="text-left">
                        <h4 className="text-sm font-bold text-white uppercase tracking-tight">Crystal of Connection</h4>
                        <p className="text-[10px] text-slate-500">Link your Google AI Studio account</p>
                      </div>
                    </div>
                    {hasKey && <span className="text-[10px] text-emerald-500 font-bold uppercase">Linked</span>}
                  </div>
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${
                      hasKey 
                      ? 'bg-slate-800 text-slate-400 border border-slate-700 hover:text-white hover:border-sky-500' 
                      : 'bg-sky-600 hover:bg-sky-500 text-white shadow-lg shadow-sky-900/20'
                    }`}
                  >
                    {hasKey ? "Re-Link Essence" : "Connect Google Account"}
                  </button>
                  ...
                  <div className="mt-4 flex items-center justify-center gap-2">
                    <a 
                      ...
                      className="text-[10px] text-slate-600 hover:text-sky-400 flex items-center gap-1 transition-colors"
                    >
                      View Billing Requirements <ExternalLink size={10} />
                    </a>
                  </div>
                </div>
              </div>
          ...
          <section className="flex-1 flex flex-col p-4 lg:p-6 bg-[#0c111d] relative">
                ...
                <div className="max-w-2xl w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  {gameState.sessionMode === 'online' && !gameState.sessionId ? (
                    ...
                    <div className="space-y-8">
                      ...
                      {!hasKey && (
                        <div className="bg-sky-500/10 border border-sky-500/30 rounded-xl p-4 flex items-center gap-3 text-left">
                          <AlertCircle className="text-sky-500 shrink-0" size={20} />
                          <p className="text-xs text-slate-300">Linking your Google Account is recommended for the best experience with Gemini 3 Pro.</p>
                        </div>
                      )}
            ...
            {error && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 bg-rose-500/90 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-2xl animate-in slide-in-from-top-2">
                ...
                <button onClick={() => setError(null)} className="ml-2 hover:text-white/70">×</button>
              </div>
            )}
      ...
      <footer className="bg-slate-900 border-t border-slate-800 p-2 flex justify-between items-center text-[10px] text-slate-500 font-mono">
        <div className="flex gap-4 items-center">
          ...
          <span className={`flex items-center gap-1 ${hasKey ? 'text-sky-400' : 'text-slate-600'}`}>
            <Battery size={10} /> SAGA_CREDITS: {remainingCredits}% {hasKey ? '(LIVE)' : '(LOCKED)'}
          </span>
          <span className="text-slate-700">|</span>
